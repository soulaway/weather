...
public class WeatherQueryServiceImpl implements WeatherQueryService , Serializable{
    ...
    /** all known airports */
    protected static List<AirportDto> AirportDto = new ArrayList<>();

    /** atmospheric information for each airport, idx corresponds with AirportDto */
     ...
     */
    public static Map<AirportDto, Integer> requestFrequency = new HashMap<AirportDto, Integer>();

    public static Map<Double, Integer> radiusFreq = new HashMap<Double, Integer>();

    
	@Override
	public List<AirportDto> getAirportData() {
		return AirportDto;
	}
	...
	public List<AtmosphericInformation> getWeather(String iata, String radiusString) {
        ...
        if (radius == 0) {
            ...
            AirportDto ad = findAirportData(iata);
            for (int i=0;i< AirportDto.size(); i++){
                if (calculateDistance(ad, AirportDto.get(i)) <= radius){
	...
	@Override
    public AirportDto findAirportData(String iataCode) {
        return AirportDto.stream()
	...
	public int getAirportDataIdx(String iataCode) {
        AirportDto ad = findAirportData(iataCode);
        return AirportDto.indexOf(ad);
	}
	...
	public Map<String, Object> getHelthStatus() {
        ...
        // fraction of queries
        for (AirportDto data : AirportDto) {
    ...
    private void updateRequestFrequency(String iata, Double radius) {
        AirportDto AirportDto = findAirportData(iata);
        requestFrequency.put(AirportDto, requestFrequency.getOrDefault(AirportDto, 0) + 1);
     ...
     */
    private double calculateDistance(AirportDto ad1, AirportDto ad2) {
